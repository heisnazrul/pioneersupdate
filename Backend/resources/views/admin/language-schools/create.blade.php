...
<div class="main-content py-10">
    <div class="flex justify-between py-10">
        <h2 class="text-2xl font-bold mb-4">Create School</h2>
    ...
    <form action="{{ route('admin.language-schools.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="accreditation_ids[]" id="accreditation_ids" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" multiple>
                @foreach($accreditations as $accreditation)
                    <option value="{{ $accreditation->id }}" {{ in_array($accreditation->id, old('accreditation_ids', [])) ? 'selected' : '' }}>{{ $accreditation->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" step="0.1" min="0" max="5" name="rating" id="rating" value="{{ old('rating', 0) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>

        <div class="flex items-center gap-2 mt-2">
            <input type="hidden" name="is_preferred" value="0">
            <input type="checkbox" name="is_preferred" value="1" id="is_preferred" class="rounded border-gray-300 text-primary focus:ring-primary" {{ old('is_preferred') ? 'checked' : '' }}>
