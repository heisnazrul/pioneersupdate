...
@section('content')
    <div class="main-content py-10">
        <div class="flex justify-between py-10">
            <h2 class="text-2xl font-bold mb-4">Edit Blog Tag</h2>
        ...
        <form action="{{ route('admin.blog-tags.update', $tag) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="ar_name" id="ar_name" value="{{ old('ar_name', $tag->ar_name) }}"
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                <div class="form-group">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="mt-1 block w-full border rounded-md px-3 py-2">{{ old('description', $tag->description) }}</textarea>
                </div>
                <div class="form-group">
                    ...
                    <textarea name="ar_description" id="ar_description" rows="3"
                        class="mt-1 block w-full border rounded-md px-3 py-2">{{ old('ar_description', $tag->ar_description) }}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-group">
                    ...
                    <input type="number" name="display_order" id="display_order"
                        value="{{ old('display_order', $tag->display_order) }}" min="0"
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                <div class="form-group flex items-center gap-3 mt-6">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" value="1" id="is_active"
                        class="rounded border-gray-300 text-primary focus:ring-primary" {{ old('is_active', $tag->is_active ? '1' : '0') === '1' ? 'checked' : '' }}>
