...
@php
    $statItems = $stats ?? [];
    $statItemsAr = $statsAr ?? [];
    $statCount = max(count($statItems), 4);
...
<div class="main-content py-10">
    <div class="flex justify-between items-center py-6">
        <div>
            ...
            <h2 class="text-2xl font-bold">Agents Page</h2>
            <p class="text-sm text-gray-500 mt-1">Static content only.</p>
        </div>
    </div>

    <div class="bg-white shadow rounded-lg p-6 mb-6">
        <form method="POST" action="{{ route('admin.cms.university.agents.update') }}" class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="border border-gray-200 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Primary CTA (EN)</label>
                        <input name="hero[cta_primary]" value="{{ old('hero.cta_primary', $hero['cta_primary'] ?? '') }}" class="mt-1 w-full border rounded px-3 py-2">
                        <input name="hero[cta_primary_ar]" value="{{ old('hero.cta_primary_ar', $heroAr['cta_primary'] ?? '') }}" class="mt-2 w-full border rounded px-3 py-2" placeholder="AR" dir="rtl">
                        <input name="hero[cta_primary_link]" value="{{ old('hero.cta_primary_link', $hero['cta_primary_link'] ?? '') }}" class="mt-2 w-full border rounded px-3 py-2" placeholder="Link">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Secondary CTA (EN)</label>
                        <input name="hero[cta_secondary]" value="{{ old('hero.cta_secondary', $hero['cta_secondary'] ?? '') }}" class="mt-1 w-full border rounded px-3 py-2">
                        <input name="hero[cta_secondary_ar]" value="{{ old('hero.cta_secondary_ar', $heroAr['cta_secondary'] ?? '') }}" class="mt-2 w-full border rounded px-3 py-2" placeholder="AR" dir="rtl">
                        <input name="hero[cta_secondary_link]" value="{{ old('hero.cta_secondary_link', $hero['cta_secondary_link'] ?? '') }}" class="mt-2 w-full border rounded px-3 py-2" placeholder="Link">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Background Image URL</label>
                        <input name="hero[bg_image]" value="{{ old('hero.bg_image', $hero['bg_image'] ?? '') }}" class="mt-1 w-full border rounded px-3 py-2">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @for($i = 0; $i < $statCount; $i++)
                        <div class="border rounded-lg p-3">
                            ...
                            <input name="stats[{{ $i }}][value]" value="{{ old("stats.$i.value", $statItems[$i]['value'] ?? '') }}" class="w-full border rounded px-2 py-1 mb-1">
                            <label class="text-xs font-semibold text-gray-600">Label (EN)</label>
                            <input name="stats[{{ $i }}][label]" value="{{ old("stats.$i.label", $statItems[$i]['label'] ?? '') }}" class="w-full border rounded px-2 py-1 mb-1">
                            <label class="text-xs font-semibold text-gray-600">Label (AR)</label>
                            <input name="stats[{{ $i }}][label_ar]" value="{{ old("stats.$i.label_ar", $statItemsAr[$i]['label'] ?? '') }}" class="w-full border rounded px-2 py-1" dir="rtl">
                        </div>
                    @endfor
                </div>
            </div>
            ...
            <div>
                <button type="submit" class="ti-btn ti-btn-outline ti-btn-outline-success rounded-full">Save Agents Page</button>
            </div>
        </form>
    </div>
</div>
