...
<div class="main-content py-10">
    ...
    <form action="{{ route('admin.language-school-pickups.update', $pickup) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="branch_id" id="branch_id"
                ...
                @foreach($branches as $branch)
                    <option value="{{ $branch->id }}" @selected(old('branch_id', $pickup->branch_id) == $branch->id)>
        ...
        <div>
            ...
            <input type="text" name="route" id="route" value="{{ old('route', $pickup->route) }}"
        ...
        <div>
            ...
            <input type="number" step="0.01" min="0" name="price" id="price" value="{{ old('price', $pickup->price) }}"
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm sm:text-sm" required>
        </div>

        <div>
            <label for="notes" class="block text-sm font-medium text-gray-700">Notes</label>
            <input type="text" name="notes" id="notes" value="{{ old('notes', $pickup->notes) }}"
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm sm:text-sm">
        </div>
