...
<div class="main-content py-10">
  <div class="flex justify-between py-10">
    ...
    <a href="{{ route('admin.language-school-course-fees.create') }}" class="ti-btn rounded-full ti-btn-outline ti-btn-outline-primary">Add Fee</a>
  </div>
  ...
  <div class="bg-white rounded-lg shadow-md p-4 mb-6">
    <form method="GET" action="{{ route('admin.language-school-course-fees.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div>
      ...
      <div class="md:col-span-4 flex items-end gap-2">
        ...
        <a href="{{ route('admin.language-school-course-fees.index') }}" class="ti-btn rounded-full border">Reset</a>
      </div>
    </form>
  </div>
        ...
        <tr class="border-b">
          ...
          <td class="px-6 py-4 text-sm">
            <div class="flex items-center gap-3">
              <a href="{{ route('admin.language-school-course-fees.create', ['language_school_course_id' => $course->id]) }}" class="text-blue-600 hover:text-blue-800">Edit Weeks</a>
              <form action="{{ route('admin.language-school-course-fees.destroy-by-course', $course) }}" method="POST" class="inline" onsubmit="return confirm('Delete all fee entries for this course?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="text-red-600 hover:text-red-800">Delete All Fees</button>
              </form>
            </div>
          </td>
        </tr>
