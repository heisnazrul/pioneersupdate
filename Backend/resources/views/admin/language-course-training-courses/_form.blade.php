...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-3">
    ...
    <div>
        ...
        <select name="branch_id" class="mt-1 w-full border rounded px-3 py-2">
            <option value="">None</option>
            @foreach($branches as $b)
                <option value="{{ $b->id }}" {{ (int)old('branch_id', $course->branch_id ?? 0) === $b->id ? 'selected' : '' }}>{{ $b->slug }}</option>
            @endforeach
        </select>
    </div>
    <div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
    <div>
        ...
        @php $ft = old('fee_type', $course->fee_type ?? 'weekly'); @endphp
