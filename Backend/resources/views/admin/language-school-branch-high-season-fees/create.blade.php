  ...
  <form action="{{ route('admin.language-school-branch-high-season-fees.store') }}" method="POST" class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input type="date" name="week_start" value="{{ old('week_start') }}" class="mt-1 block w-full border rounded-md px-3 py-2" required>
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Week End Date (optional)</label>
        <input type="date" name="week_end" value="{{ old('week_end') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
    </div>

    <div>
      <label class="block text-sm font-medium text-gray-700">Fee</label>
      <input type="number" step="0.01" min="0" name="fee" value="{{ old('fee') }}" class="mt-1 block w-full border rounded-md px-3 py-2" required>
    </div>
