...
<div class="main-content py-10">
  ...
  <form action="{{ route('admin.language-school-accommodations.store') }}" method="POST" class="space-y-6">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        <label class="block text-sm font-medium text-gray-700">Title (EN)</label>
        <input type="text" name="title" value="{{ old('title') }}" class="mt-1 block w-full border rounded-md px-3 py-2" required>
      </div>
