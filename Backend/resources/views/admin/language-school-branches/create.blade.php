...
<div class="main-content py-10">
  <div class="flex justify-between py-10">
    <h2 class="text-2xl font-bold mb-4">Create School Branch</h2>
  ...
  <form action="{{ route('admin.language-school-branches.store') }}" method="POST" class="space-y-4">
    @csrf
    ...
    <div>
      ...
      <select name="language_school_id" class="mt-1 block w-full border rounded-md px-3 py-2" required>
        ...
        @foreach($schools as $school)
          <option value="{{ $school->id }}" @selected(old('language_school_id', old('school_id')) == $school->id)>{{ $school->name }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div>
      ...
      <textarea name="gallery_urls" class="mt-1 block w-full border rounded-md px-3 py-2" rows="5">{{ old('gallery_urls') }}</textarea>
    </div>

    <div>
      <label class="block text-sm font-medium text-gray-700">Video URL</label>
      <input type="text" name="video_url" value="{{ old('video_url') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
    </div>
