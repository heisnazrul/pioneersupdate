...
<div class="box border border-gray-200 dark:border-white/10 mb-4">
    <div class="box-header bg-gray-50 dark:bg-black/20">
        <div class="box-title">Why Partner With Us?</div>
    </div>
    <div class="box-body">
        <div class="grid grid-cols-12 gap-6 mb-4">
            <div class="col-span-12">
                <label class="{{ $labelClass }}">Section Title</label>
                <input type="text" class="{{ $inputClass }}" name="content[benefits][title]"
            ...
            <div class="border border-gray-200 dark:border-white/10 p-4 mb-3 rounded-sm">
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 md:col-span-4">
                        ...
                        <input type="text" class="{{ $inputClass }}" name="content[benefits][items][{{$index}}][title]"
                            value="{{ $item['title'] ?? '' }}">
                    </div>
                    <div class="col-span-12 md:col-span-8">
                        <label class="{{ $labelClass }}">Description</label>
                        <input type="text" class="{{ $inputClass }}"
                            name="content[benefits][items][{{$index}}][description]"
...
<div class="box border border-gray-200 dark:border-white/10 mb-4">
    <div class="box-header bg-gray-50 dark:bg-black/20">
        <div class="box-title">Joining Process</div>
    </div>
    <div class="box-body">
        <div class="grid grid-cols-12 gap-6 mb-4">
            <div class="col-span-12">
                <label class="{{ $labelClass }}">Section Title</label>
                <input type="text" class="{{ $inputClass }}" name="content[how_to_join][title]"
                    value="{{ $content['how_to_join']['title'] ?? '' }}">
            </div>
        </div>

        <h6 class="font-bold mb-3 text-gray-800 dark:text-white">Steps</h6>
        @foreach($content['how_to_join']['steps'] ?? [] as $index => $step)
            <div class="border border-gray-200 dark:border-white/10 p-4 mb-3 rounded-sm">
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 md:col-span-2">
                        <label class="{{ $labelClass }}">Number</label>
                        <input type="text" class="{{ $inputClass }}" name="content[how_to_join][steps][{{$index}}][number]"
                            value="{{ $step['number'] ?? '' }}">
                    </div>
                    <div class="col-span-12 md:col-span-4">
                        <label class="{{ $labelClass }}">Title</label>
                        <input type="text" class="{{ $inputClass }}" name="content[how_to_join][steps][{{$index}}][title]"
                            value="{{ $step['title'] ?? '' }}">
                    </div>
                    <div class="col-span-12 md:col-span-6">
                        <label class="{{ $labelClass }}">Description</label>
                        <input type="text" class="{{ $inputClass }}"
                            name="content[how_to_join][steps][{{$index}}][description]"
