    ...
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        <div class="flex flex-col gap-6">
            <div class="flex justify-between items-center">
                <h5 class="text-xl font-semibold mb-0">Edit Subject Area: {{ $subjectArea->name }}</h5>
            ...
            <div class="box shadow-sm border border-gray-200 dark:border-white/10 rounded-lg overflow-hidden">
                <div class="box-body !p-6">
                    <form action="{{ route('admin.subject-areas.update', $subjectArea->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        @include('admin.subject_areas._form')

                        <div class="flex justify-end mt-6 gap-2">
                            <button type="submit" class="ti-btn ti-btn-primary !m-0">Update Subject Area</button>
                        </div>
                    </form>
                </div>
            </div>
