...
<div class="main-content py-10">
  <div class="flex justify-between py-10">
    <h2 class="text-2xl font-bold mb-4">Create Language Course</h2>
  ...
  <form action="{{ route('admin.language-school-courses.store') }}" method="POST" class="space-y-4">
    @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input type="text" name="start_day" value="{{ old('start_day') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
    </div>
    ...
    <div>
      ...
      <textarea name="description" class="mt-1 block w-full border rounded-md px-3 py-2" rows="4">{{ old('description') }}</textarea>
    </div>

    <div>
      <label class="block text-sm font-medium text-gray-700">Description (AR)</label>
      <textarea name="ar_description" class="mt-1 block w-full border rounded-md px-3 py-2" rows="4">{{ old('ar_description') }}</textarea>
    </div>
      ...
      <div>
        ...
        <select name="language_course_type_id" class="mt-1 block w-full border rounded-md px-3 py-2" required>
          ...
          @foreach($types as $type)
            <option value="{{ $type->id }}" @selected(old('language_course_type_id') == $type->id)>{{ $type->name }}</option>
          @endforeach
        </select>
      </div>
      <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input type="text" name="required_level" value="{{ old('required_level') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Study Time</label>
        <input type="text" name="study_time" value="{{ old('study_time') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input type="text" name="lessons_per_week" value="{{ old('lessons_per_week') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Min Age</label>
        <input type="text" name="min_age" value="{{ old('min_age') }}" class="mt-1 block w-full border rounded-md px-3 py-2">
      </div>
    </div>
