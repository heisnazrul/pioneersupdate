    ...
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        <div class="flex flex-col gap-6">
            <div class="flex justify-between items-center">
                <h5 class="text-xl font-semibold mb-0">Study Levels</h5>
                <a href="{{ route('admin.levels.create') }}" class="ti-btn ti-btn-primary !m-0">
                    <i class="ri-add-line mr-1"></i> Add New Level
                </a>
            </div>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-white/10">
                                @forelse($levels as $level)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-black/20">
                                        <td class="px-6 py-4 text-sm text-gray-500">{{ $level->sort_order }}</td>
                                        <td class="px-6 py-4 text-sm font-medium text-gray-800 dark:text-gray-200">
                                            {{ $level->name }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-500 font-mono">{{ $level->key }}</td>
                                        <td class="px-6 py-4">
                                            <span
                                                class="badge {{ $level->is_active ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger' }} rounded-full px-3 py-1 text-xs">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                            <i class="ri-database-2-line text-4xl text-gray-300 mb-2 block"></i>
                                            No levels found.
                                        </td>
                                    </tr>
                                @endforelse
