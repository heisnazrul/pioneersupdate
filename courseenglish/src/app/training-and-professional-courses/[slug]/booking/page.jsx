...
import { faShare, faCheckCircle, faPencilAlt, faHeart, faExchangeAlt, faCalendarAlt, faUser, faClock } from "@fortawesome/free-solid-svg-icons";
...
/* ── Content Component ────────────────────────────────────── */
function TrainingBookingContent({ slug }) {
    ...
    // Fetch data 
    const { data, loading, error } = useApi(slug ? `/courseenglish/training-and-professional-courses/${slug}` : null);
    ...
    const handleBookingSubmit = async (verificationToken = null) => {
        ...
        try {
            ...
            const res = await fetch(buildApiUrl("/courseenglish/booking/training-course"), {
                method: "POST",
                headers: headers,
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="mb-8 flex flex-col lg:flex-row items-center justify-between gap-4">
                    ...
                    <div className="w-full lg:w-auto text-center lg:text-end relative">
                        <Link href={`/training-and-professional-courses/${slug}`} className="lg:hidden absolute top-1 left-0 text-slate-600 p-2">
                            <Image src="/assets/icons/arrow-left.svg" width={20} height={20} alt="Back" className={isArabic ? "" : "rotate-180"} />
                        </Link>
                        <Link href={`/training-and-professional-courses/${slug}`} className="hidden lg:inline-flex items-center gap-2 text-sm font-medium text-slate-600 mb-4 transition hover:text-[#0057B7]">
                    ...
                    <div className="order-1 lg:order-2 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-4 flex gap-4 items-start shadow-sm">
                            ...
                            <div className="flex-1 flex flex-col items-start justify-center h-full py-1">
                                <div className="w-full flex justify-between items-start mb-1">
                                    <Link href={`/training-and-professional-courses/${slug}`} className="bg-gray-50 text-slate-500 text-[10px] font-medium px-3 py-1 rounded hover:bg-gray-100 transition whitespace-nowrap">
                                        {l("change", isArabic)}
                                    </Link>
                            ...
                            <div className="space-y-4">
                                {course && (
                                    <div className="flex justify-between items-center text-sm group gap-6">
                                        ...
                                        <span className="text-slate-600 font-normal text-end flex-1 ">{t(course.name, course.ar_name, isArabic)}</span>
                                    </div>
                                )}
                        ...
                        <div className="bg-[#F8FAFC] rounded-2xl border border-gray-50 p-5">
                            <div className="flex justify-between items-center mb-4">
                                <Link href={`/training-and-professional-courses/${slug}`} className="bg-white border border-gray-100 text-slate-500 text-[10px] font-medium px-3 py-1 rounded hover:bg-gray-50 transition shadow-sm">
                                    {l("change", isArabic)}
                                </Link>
                                <h4 className="font-medium text-slate-900 text-sm">{l("courseDetails", isArabic)}</h4>
                            </div>
                            <div className="text-end space-y-1">
                                <p className="font-medium text-slate-800 text-sm">{course ? t(course.name, course.ar_name, isArabic) : "-"}</p>
                                <p className="text-xs text-slate-500 font-normal">{l("startDate", isArabic)}: {formattedStartDate}</p>
                                {course?.duration && (
                                    <p className="text-xs text-slate-500 font-normal">{l("duration", isArabic)}: {course.duration} {l("weeks", isArabic)}</p>
                                )}
                            </div>
                        </div>
