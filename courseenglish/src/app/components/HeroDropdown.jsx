...
export default function HeroDropdown({ label, placeholder, options = [], scroll = false, onSelect, selectedValue }) {
    ...
    return (
        ...
        >
            <summary
                className={`flex w-full cursor-pointer items-center justify-between gap-3 rounded-xl border bg-white px-8 py-4 sm:px-5 transition list-none [&::-webkit-details-marker]:hidden ${isOpen ? "border-blue-300 ring-2 ring-blue-100" : "border-[#F6F8FA] shadow-sm"}`}
                style={{ listStyle: "none" }}
            >
                <div className="min-w-0 flex flex-col ">
                    <span className="truncate whitespace-nowrap text-[13px] font-normal text-slate-700 sm:text-sm">{label}</span>
                ...
                {options.map((option, idx) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full rounded-xl px-3 py-2  text-sm ${isSelected
