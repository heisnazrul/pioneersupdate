...
import { faShare, faCheckCircle, faPencilAlt, faHeart, faExchangeAlt, faBookOpen, faClock, faUser, faSignal } from "@fortawesome/free-solid-svg-icons";
...
/* ── Content Component ────────────────────────────────────── */
function OnlineBookingContent({ slug }) {
    ...
    // Fetch data (using details endpoint to get course info)
    // Note: The details endpoint returns { school, courses: [] }
    const { data, loading, error } = useApi(slug ? `/courseenglish/online-courses/${slug}` : null);
    ...
    const coursePrice = selectedCourse
        ? (currency === "SAR" ? selectedCourse.price_sar : selectedCourse.price_gbp) * weeks
        : 0;

    const totalPrice = coursePrice;

    // Helpers for rendering
    const cityName = school.city_name || school.city || school.location || "";
    const cityNameAr = school.city_ar || school.city_ar_name || cityName;
    const schoolName = isArabic
        ? [school.ar_name, cityNameAr, school.name].filter(Boolean).join(" - ")
        : school.name;
    ...
    const handleBookingSubmit = async (verificationToken = null) => {
        ...
        try {
            ...
            const res = await fetch(buildApiUrl("/courseenglish/booking/online-course"), {
                method: "POST",
                headers: headers,
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="mb-8 flex flex-col lg:flex-row items-center justify-between gap-4">
                    ...
                    <div className="w-full lg:w-auto text-center lg:text-end relative">
                        <Link href={`/online-courses/${slug}`} className="lg:hidden absolute top-1 left-0 text-slate-600 p-2">
                            <Image src="/assets/icons/arrow-left.svg" width={20} height={20} alt="Back" className={isArabic ? "" : "rotate-180"} />
                        </Link>
                        <Link href={`/online-courses/${slug}`} className="hidden lg:inline-flex items-center gap-2 text-sm font-medium text-slate-600 mb-4 transition hover:text-[#0057B7]">
                    ...
                    <div className="order-1 lg:order-2 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-4 flex gap-4 items-start shadow-sm">
                            ...
                            <div className="flex-1 flex flex-col items-start justify-center h-full py-1">
                                <div className="w-full flex justify-between items-start mb-1">
                                    <Link href={`/online-courses/${slug}`} className="bg-gray-50 text-slate-500 text-[10px] font-medium px-3 py-1 rounded hover:bg-gray-100 transition whitespace-nowrap">
                                        {l("change", isArabic)}
                                    </Link>
                            ...
                            <div className="space-y-4">
                                {selectedCourse && (
                                    <div className="flex justify-between items-center text-sm group gap-6">
                                        <span className="font-medium text-slate-900 tabular-nums shrink-0">{formatCurrency(coursePrice, currency)}</span>
                        ...
                        <div className="bg-[#F8FAFC] rounded-2xl border border-gray-50 p-5">
                            <div className="flex justify-between items-center mb-4">
                                <Link href={`/online-courses/${slug}`} className="bg-white border border-gray-100 text-slate-500 text-[10px] font-medium px-3 py-1 rounded hover:bg-gray-50 transition shadow-sm">
                                    {l("change", isArabic)}
                                </Link>
                            ...
                            <div className="text-end space-y-1">
                                ...
                                <div className="mt-2 text-xs font-medium text-slate-500 flex flex-wrap gap-2 justify-end">
                                    {selectedCourse?.lessons_per_week && (
                                        <span className="flex items-center gap-1">
                                            {selectedCourse.lessons_per_week} {l("lessons", isArabic)}
                                            <FontAwesomeIcon icon={faBookOpen} className="text-[#0057B7]" />
                                        </span>
                                    )}
                                    {selectedCourse?.study_time && (
                                        <span className="flex items-center gap-1">
                                            {selectedCourse.study_time} {l("hours", isArabic)}
                                            <FontAwesomeIcon icon={faClock} className="text-[#0057B7]" />
                                        </span>
                                    )}
                                </div>
                            </div>
