...
class CourseEnglishApiSupport
{
    ...
    public function languageCourseCard(LanguageSchoolCourse $course): array
    {
        ...
        return [
            ...
            'school_slug' => $branch?->slug ?: $school?->slug,
            ...
            'country_name' => $country?->name,
            'country_ar_name' => $country?->ar_name,
            'country_flag' => $this->toPublicUrl($country?->flag),
            'flag' => $this->toPublicUrl($country?->flag),
            'logo' => $this->toPublicUrl($school?->logo_url),
            ...
            'url' => $branch?->slug ? "/language-institutes/{$branch->slug}?course_id={$course->id}" : null,
        ];
    }
    ...
    {
        $weekly = $this->toGbpSar($accommodation->weekly_fee, 'GBP');
        $admin = $this->toGbpSar($accommodation->admin_fee, 'GBP');
        ...
        return [
            ...
            'fee_per_week_gbp' => $weekly['gbp'],
            'fee_per_week_sar' => $weekly['sar'],
            'admin_fee' => $accommodation->admin_fee,
            'admin_fee_gbp' => $admin['gbp'],
            'admin_fee_sar' => $admin['sar'],
    ...
    {
        $fees = $this->toGbpSar($pickup->fee, 'GBP');
    ...
    {
        $weekly = $this->toGbpSar($insurance->weekly_fee, 'GBP');
        $admin = $this->toGbpSar($insurance->admin_fee, 'GBP');

        return [
            'id' => $insurance->id,
            'name' => 'Insurance',
            'amount' => $insurance->weekly_fee,
            'amount_gbp' => $weekly['gbp'],
            'amount_sar' => $weekly['sar'],
            'price' => $insurance->weekly_fee,
            'price_gbp' => $weekly['gbp'],
            'price_sar' => $weekly['sar'],
    ...
    public function faqCard(Faq $faq): array
    {
        return [
            'id' => $faq->id,
            'cat' => $faq->category,
            'cat_ar' => $faq->ar_category,
            'q' => $faq->question,
            'q_ar' => $faq->ar_question,
            'a' => $faq->answer,
            'a_ar' => $faq->ar_answer,
        ];
    }
