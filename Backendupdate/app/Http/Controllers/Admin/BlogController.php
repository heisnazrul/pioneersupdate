...
class BlogController extends Controller
{
    ...
    {
        $blogs = Blog::with(['category', 'publisher'])
    ...
    private function validateRequest(Request $request, ?Blog $blog = null): array
    {
        ...
        $data = $request->validate([
            ...
            'slug'                  => ['nullable', 'string', 'max:255', Rule::unique('blogs', 'slug')->ignore($id)->whereNull('deleted_at')],
    ...
    private function formData(?Blog $blog = null): array
    {
        return [
            'blog'           => $blog,
            'categories'     => BlogCategory::active()->orderBy('display_order')->orderBy('name')->get(),
            'blogTags'       => BlogTag::active()->orderBy('display_order')->orderBy('name')->get(),
