...
@section('header', 'Edit Accreditation')
...
<div class="max-w-4xl" x-data="mediaPicker()">
    ...
    <form action="{{ route('admin.accreditations.update', $accreditation) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
...
function mediaPicker() {
    return {
        ...
        selectedId: null,
        tempSelected: null,
        selectedUrl: '{{ $accreditation->logo ? Storage::url($accreditation->logo) : "" }}',
        selectedPath: '{{ $accreditation->logo ?? "" }}',
        uploadFile: null,
        uploadPreview: null,
        uploadTitle: '',
