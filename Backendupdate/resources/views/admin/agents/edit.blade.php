...
@section('header', 'Edit Affiliate Agent')
...
<div class="max-w-4xl">
    ...
    <form action="{{ route('admin.agents.update', $agent) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        @include('admin.agents._form', ['agent' => $agent, 'userStatuses' => $userStatuses])
        <div class="flex justify-end">
            <button type="submit" class="rounded-lg bg-primary-600 px-8 py-3 font-semibold text-white shadow-lg hover:bg-primary-700">Save Changes</button>
        </div>
    </form>
</div>
@endsection
