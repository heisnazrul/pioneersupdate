...
@section('title', 'Course Compares')
@section('header', 'Course Compares')

@section('content')
<div class="mb-6">
    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Active Compares</h3>
</div>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                ...
                <td class="px-6 py-4 text-right">
                    <form action="{{ route('admin.compares.destroy', $item) }}" method="POST" onsubmit="return confirm('Remove from compare?')">
                        @csrf
                        @method('DELETE')
            ...
            @empty
            <tr><td colspan="5" class="px-6 py-10 text-center text-gray-500 italic">No compare items found.</td></tr>
            @endforelse
        </tbody>
    </table>
    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-750 border-t border-gray-100 dark:border-gray-700">{{ $compares->links() }}</div>
</div>
@endsection
