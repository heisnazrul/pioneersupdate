...
@section('title', 'Edit School Branch')
...
<div class="max-w-4xl" x-data="branchPicker()">
    ...
    <form action="{{ route('admin.language-school-branches.update', $languageSchoolBranch) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 rounded-xl p-6 space-y-6">
                ...
                <div>
                    ...
                    <select name="school_id" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                        @foreach($schools as $school)
                            <option value="{{ $school->id }}" {{ old('school_id', $languageSchoolBranch->school_id) == $school->id ? 'selected' : '' }}>{{ $school->name_en }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="about_en" rows="4" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">{{ old('about_en', $languageSchoolBranch->about_en) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">About (Arabic)</label>
                    <textarea name="about_ar" rows="4" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white" dir="rtl">{{ old('about_ar', $languageSchoolBranch->about_ar) }}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="new_year_close_from" value="{{ old('new_year_close_from', $languageSchoolBranch->new_year_close_from?->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">New Year Close To</label>
                    <input type="date" name="new_year_close_to" value="{{ old('new_year_close_to', $languageSchoolBranch->new_year_close_to?->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="is_active" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    <option value="yes" {{ old('is_active', $languageSchoolBranch->is_active) == 'yes' ? 'selected' : '' }}>Yes</option>
                    <option value="no" {{ old('is_active', $languageSchoolBranch->is_active) == 'no' ? 'selected' : '' }}>No</option>
                </select>
            </div>
        </div>

        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.language-school-branches.index') }}" class="px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 transition-colors">
                Cancel
            </a>
    ...
    <!-- Multi Media Picker Modal (Same as create) -->
                ...
                <div class="flex-1 flex flex-col p-6 overflow-hidden">
                    <div class="flex gap-4 mb-6">
                        <div class="flex-1 relative">
                            ...
                            <input type="text" x-model="searchTerm" @input.debounce.300ms="fetchImages()" placeholder="Search branch images..." class="w-full pl-10 pr-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-primary-500 bg-gray-50 dark:bg-gray-900 text-sm">
                        </div>
                    ...
                    <div x-show="tab === 'upload'" class="flex-1 flex flex-col items-center justify-center">
                        <div class="w-full max-w-md space-y-4">
                            ...
                            <input type="text" x-model="uploadTitle" placeholder="Image Title" class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                            <div class="flex justify-between items-center pt-4">
                                <button type="button" @click="tab = 'gallery'" class="text-sm text-gray-500 hover:text-gray-700">Back</button>
                                <button type="button" @click="uploadAndSelect()" :disabled="!uploadFile || uploading" class="bg-primary-600 text-white px-6 py-2 rounded-lg text-sm font-medium shadow-md">
...
function branchPicker() {
    return {
        selectedCityId: '{{ old("city_id", $languageSchoolBranch->city_id) }}',
        selectedCityName: '{{ $languageSchoolBranch->city->name }} ({{ $languageSchoolBranch->city->country->name }})',
        ...
        selectedImages: [
            @if($languageSchoolBranch->branch_images)
                @foreach($languageSchoolBranch->branch_images as $img)
                    { id: '{{ md5($img) }}', path: '{{ $img }}', url: '{{ Storage::url($img) }}' },
                @endforeach
            @endif
        ],
        ...
        toggleImage(img) {
            const index = this.tempSelected.findIndex(i => i.path === img.path);
            if (index > -1) {
