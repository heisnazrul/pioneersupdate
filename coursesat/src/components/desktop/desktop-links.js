  ...
  return (
    <section className="hidden md:block bg-[#F3F7FF] py-16 sm:py-20 w-full" dir={direction}>
      <div className="px-6 md:px-10 xl:px-20 2xl:px-40 mx-auto">
        ...
        {/* Link Grid */}
        <div className="grid grid-cols-4 gap-x-12 text-[14px] text-slate-700 leading-relaxed text-start">
            ...
            <div key={colIdx} className="flex flex-col gap-2">
              {col.map((item, idx) => (
                <a
                  key={`${item.label}-${idx}`}
                  href={item.url || "#"}
                  className="transition-colors hover:text-blue-600 block hover:underline"
                >
                  {item.label}
                </a>
              ))}
