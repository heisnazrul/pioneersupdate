...
const MENU = [
  ...
  { href: "/student/bookings", icon: faBookmark, key: "bookings" },
...
export default function StudentShell({ children }) {
  ...
  const loc = (key, fallback = "") => t(`pages.student.nav.${key}`, fallback);
    ...
    <div className="flex min-h-screen flex-col bg-[#F2F4F7]">
      ...
      <footer className="border-t border-slate-200 bg-white">
        ...
        >
          ...
          <span className="text-slate-400">CourseEnglish</span>
        </div>
      </footer>
    </div>
  );
...
function StudentSidebar({ isRtl, isActive, logout, loc, onNavigate }) {
  return (
