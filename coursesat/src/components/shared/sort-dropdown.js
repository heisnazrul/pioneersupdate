...
export default function SortDropdown({ tags = [], onSortChange }) {
    ...
    const parts = [];
    if (selectedTag) {
        const tObj = tags.find((tItem) => typeof tItem === "string" ? tItem === selectedTag : tItem.name === selectedTag);
        const name = typeof tObj === "string" ? tObj : (isArabic ? (tObj?.ar_name || selectedTag) : selectedTag);
        parts.push(name);
    }
    ...
    return (
        <details ref={detailsRef} className="relative z-30 w-fit">
            <summary
                className="flex items-center gap-3 rounded-xl border border-[#DCE6F1] bg-white px-4 py-3 cursor-pointer select-none [&::-webkit-details-marker]:hidden"
            ...
            >
                ...
                <div className="flex items-center whitespace-nowrap text-start">
                    <span className="text-sm font-normal text-slate-500">{t("layouts.common.sort_by", isArabic ? "ترتيب حسب : " : "Sort by : ")}</span>
                    <span className="text-sm font-medium text-slate-800 ms-1">{summaryLabel}</span>
                </div>
            ...
            <div className="absolute start-0 z-40 mt-3 w-full rounded-xl border border-[#E6EEF7] bg-white p-3 shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
                ...
                {tags.length > 0 && (
                    <div className="mb-2">
                        <p className="mb-1 px-2 text-xs font-medium uppercase tracking-wider text-slate-400 text-start">
                        ...
                        {tags.map((tag) => {
                            ...
                            return (
                                <button
                                    key={tag.id || tagName}
                                    className={`w-full rounded-lg px-4 py-2.5 text-start text-sm font-normal transition ${selectedTag === tagName
