  ...
  return (
    ...
    >
      <summary
        className={
          variant === "sidebar"
            ? `block w-full cursor-pointer rounded-2xl border bg-white px-5 py-4 shadow-sm transition list-none [&::-webkit-details-marker]:hidden hover:border-gray-300 ${
                isOpen ? "border-blue-300 ring-2 ring-blue-100" : "border-gray-200"
              }`
            : variant === "borderless"
            ? "flex w-full cursor-pointer items-center justify-between gap-3 px-4 py-2 bg-transparent transition list-none [&::-webkit-details-marker]:hidden"
            : `flex w-full cursor-pointer items-center justify-between gap-3 rounded-xl border bg-white px-8 py-3 2xl:py-4 sm:px-5 transition list-none [&::-webkit-details-marker]:hidden ${
                isOpen ? "border-blue-300 ring-2 ring-blue-100" : "border-[#F6F8FA] shadow-sm"
              }`
        }
      ...
      >
        {variant === "sidebar" ? (
          <>
            <span className="block text-[12px] font-medium text-slate-500 text-start">{label}</span>
            <span
              className={`mt-1.5 block text-[14px] text-start ${
                displayLabel ? "text-[#102233]" : "text-slate-400"
              }`}
            >
          ...
          <div className="min-w-0 flex flex-col text-start">
            ...
            <span
              className={`mt-1 truncate whitespace-nowrap text-[13px] sm:text-sm ${
      ...
      <div
        className={`absolute left-0 z-[100] mt-2 w-full rounded-2xl border border-[#E6EEF7] bg-white p-2 shadow-[0_10px_30px_rgba(15,23,42,0.08)] ${
